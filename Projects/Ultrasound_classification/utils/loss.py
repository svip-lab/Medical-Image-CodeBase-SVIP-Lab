...
class dice_bce_loss(nn.Module):
    ...
    def multi_class_one_hot(self, label, classes):
        N, H, W = label.size(0), label.size(1), label.size(2)
    ...
    def __call__(self, y_true, y_pred, num_classes=4):

        y_prediction = self.softmax(y_pred)
        # print("=====================")
        # print(y_true.size(),y_pred.size(),y_prediction.size())
        # print("=====================")
        y_mask_one_hot = self.multi_class_one_hot(y_true, classes=num_classes)
